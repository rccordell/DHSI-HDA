...
census <- unite(census, sillyColumn, c("county", "state"), sep = "%%%")
...
NEStates <- c("Massachusetts","Vermont","New Hampshire","Connecticut","Rhode Island","Maine")

View(filter(census, state %in% NEStates))
...
census %>%
  select(county, state, SlavePopulation, WhitePopulation) %>%
  filter(SlavePopulation > WhitePopulation) %>%
  View()
...
census_sum %>%
  ...
  select(state, Male, Female, papers) %>%
  View()
...
write_csv(paper, file = "./output/papers.csv")
...
crewlists <- read_csv("./data/crewlists.csv") %>%
  separate(ApproximateDeparture, into = c("month", "day", "year"), sep = "/") %>%
  mutate(year = as.numeric(year)) %>%
  filter(year >= 100) %>%
  filter(!is.na(Age))

summary(crewlists$Age)
...
crewlists <- arrange(crewlists, desc(Age))
