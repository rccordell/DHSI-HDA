...
test <- address_words %>%
  ...
  count(filename, president, index = as.numeric(rownames(.)) %/% 10, sentiment) %>% #this is throwing an error (what's the floor divide 80 for?)
...
Varying `n` changes how many words are captured in each iteration. A "bi-gram" means `n = 2`, "tri-gram" means `n = 3`, "four-gram" means `n = 4`, and so on. Below, try tokenizing `sample_string` by a different n-gram for yourself, and see how it affects the result.
...
Spreading this data will show us the number of times a particular n-gram (in this case, a 4-gram) occurs in each text.

```{r}
address_ngrams %>% spread(filename,count,fill=0) %>% View()
```

Generally speaking, a larger `n` will yield much more specific n-grams that incorporate more context, but are much less likely to be shared by other texts. Making a choice for `n` is thus a balancing act between , and there is no one "right" choice: it depends on what you want to know.

How would you facet this in order to find some commonly-shared phrases

stylistic analysis (stock phrases between different presidents); 30-year increments

are there any matches (4 grams: United States of America)
