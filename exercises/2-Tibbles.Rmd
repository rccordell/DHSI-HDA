...
newspapers <- tibble("title" = c("Sedalia Daily Bazoo","Boons Lick Times","Jefferson Jimplecute"), "founded" = c(1869, 1840, 1848), frequency = c("daily","weekly","weekly"))
...
census <- select(census, contains("Newspapers"))
...
3. Determine the county with the highest population of literate whites over 20 years old, then determine the county with the highest ratio of literate whites to general population.
...
5. Reach goal: What is the oldest children's book in `booktitles-part1.tsv`?


```{r}

books <- read_tsv("./data/booktitles-part1.tsv")

census <- select(census, FreeColoredPopulation, SlavePopulation)

census <- mutate(census, ratio = as.numeric(FreeColoredPopulation / SlavePopulation))

census <- mutate(census, percent = ratio * 100)
