...
Box and whisker plots are used to visualize some of the most relevant summary data associated with chosen groupings of data on the horizontal axis.
...
See if you can spot an issue with the line chart!

# 4. Axes and Scales

Sometimes the default scale that `ggplot` chooses will not be the most elucidatory one. We can
...
Generally speaking, labels of the elements outside the plot area — such as axis titles and a title for the plot — are specified in a `labels` function that builds on the `ggplot` function. 
...
Last but not least, we'd like to link you to a potentially helpful [cheatsheat](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf)!
# Exercises