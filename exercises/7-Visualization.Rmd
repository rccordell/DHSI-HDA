...
# 2. Geometries: Types of Plots in `ggplot2`
...
This can help us see just how many works cited by DHQ authors have yet to be included in the Biblio corpus (some of these are likely duplicates!).
...
In this case, a line chart works pretty well. A word of caution, however: you might think that a line chart is always the best way to show data over time, but it isn't always! Generally, you use a line chart when you want to emphasize overall trends in the story you are trying to tell with your data. **Interpolation** between the points plotted on the chart, used to connect them with straight lines, can obscure precision of the values in your data. In fact, depending on the scale of your horizontal axis, it can look like you have plotted data that you don't have!

What if we pretended, for the sake of argument, that the journal went on a brief break in 2011? Let's compare a bar chart and line chart of the same data.

First, we'll need to install a package that lets us look at plots side by side.
...
# 3. Mapping Variables to Plot Elements
